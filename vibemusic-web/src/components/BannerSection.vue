...
function loadBanners() {
  ...
  apiBanners().then(res => {
    if (res.data && res.data.length > 0) slides.value = res.data
...
.banner-skel { position: absolute; inset: 0; border-radius: 14px; background: linear-gradient(90deg, #eee 25%, #f0f0f0 50%, #eee 75%); background-size: 200% 100%; animation: shimmer 1.5s ease-in-out infinite; }
.banner-slide { position: absolute; inset: 0; opacity: 0; transform: scale(0.96); transition: all .6s ease; cursor: pointer; border-radius: 14px; background-size: cover; background-position: 35% center; }
