...
const seekPreviewPct = ref(0) // 拖拽中的预览进度（0-1），松手前不提交 seek

function seekAt(clientX, commit = false) {
  ...
  // Apple VP0 范式：拖拽中仅更新预览，松手（commit=true）才真正 seek
  if (commit) store.seekTo(pct)
}
...
function onTrackMouseUp(e) {
  seekAt(e.clientX, true) // 松手才真正 seek
    ...
    <footer class="bar">
      <div class="progress-wrap">
        ...
        <div ref="progressTrack" class="progress-track" :class="{ seeking: isSeeking }" @mousedown="onTrackMouseDown">
          ...
          <!-- 填充条：scaleX 合成器动画；thumb 移出填充条，避免被 scaleX 压扁 -->
          <!-- 拖拽中填充/thumb 跟随预览进度，松手后回落到 store 进度 -->
          <div class="progress-fill" :style="{ '--progress': isSeeking ? seekPreviewPct : progress / 100 }"></div>
          <div class="thumb" :style="{ left: (isSeeking ? seekPreviewPct * 100 : progress) + '%' }"></div>
        </div>
      ...
      <div class="ctrl-wrap">
        <div class="left-info">
          <div
            ...
            title="歌词"
          >
            <SvgIcon v-if="!currentSong.coverUrl" name="music" />
          </div>
          <div class="mini-song">
      ...
      <div v-if="showPlaylist" ref="playlistPanelRef" class="playlist-panel">
        <div class="panel-header">
          ...
          <div class="panel-header-actions">
            ...
            <button class="panel-close" @click="showPlaylist = false"><SvgIcon name="close" /></button>
          </div>
        </div>
        <div class="panel-list">
          ...
          >
            ...
            <button class="pi-remove" @click.stop="store.removeFromQueue(idx)" title="移除"><SvgIcon name="close" /></button>
          </div>
...
.bar { padding: 10px 28px 16px; background: var(--bg-elevated); border-top: 1px solid var(--bg-hover); box-shadow: 0 -2px 12px rgba(0,0,0,.4); }

.progress-wrap { display: flex; align-items: center; gap: 14px; margin-bottom: 14px; }
.time { font-size: 13px; color: var(--text-secondary); min-width: 44px; font-variant-numeric: tabular-nums; text-align: center; }
.progress-track { flex: 1; height: 4px; background: rgba(255,255,255,.1); border-radius: 2px; cursor: pointer; position: relative; transition: height .15s; }
...
.progress-fill {
  height: 100%; background: #31c27c; border-radius: 2px; position: relative;
  /* 合成器动画：scaleX 代替 width，避免布局抖动 */
  transform: scaleX(var(--progress, 0));
  transform-origin: left;
  transition: transform .2s ease-out, filter .15s;
}
/* 拖拽中禁用 transform 过渡，填充条即时跟随鼠标 */
.progress-track.seeking .progress-fill { filter: brightness(1.2); transition: filter .15s; }

.thumb {
  position: absolute; top: 50%; left: 0;
  transform: translate(-50%, -50%);
  width: 12px; height: 12px; background: #31c27c; border-radius: 50%;
...
.mini-cover.active { border: 2px solid rgba(49,194,124,0.5); }
...
.ctrl-btn { border: none; background: none; color: var(--text-secondary); cursor: pointer; display: flex; align-items: center; justify-content: center; transition: .15s; }
.ctrl-btn:hover { color: var(--text-primary); }
.ctrl-btn.mode-btn { width: 40px; height: 40px; border-radius: 50%; color: var(--text-secondary); }
...
.act-icon { width: 38px; height: 38px; border: none; background: none; border-radius: 50%; color: var(--text-secondary); cursor: pointer; display: flex; align-items: center; justify-content: center; transition: .15s; }
.act-icon:hover { color: var(--text-primary); background: rgba(255,255,255,.08); }
...
.vol-bar { width: 70px; height: 4px; background: rgba(255,255,255,.1); border-radius: 2px; cursor: pointer; }
.vol-bar:hover { height: 6px; }
.vol-fill { height: 100%; background: var(--text-secondary); border-radius: 2px; }
...
.panel-header {
  ...
  padding: 18px 20px; border-bottom: 1px solid var(--bg-hover);
  font-size: 15px; color: var(--text-primary); flex-shrink: 0;
}
.panel-header-actions { display: flex; align-items: center; gap: 12px; }
.panel-clear-btn { background: none; border: 1px solid var(--bg-hover); border-radius: 4px; color: var(--text-secondary); font-size: 12px; padding: 2px 8px; cursor: pointer; }
.panel-clear-btn:hover { color: #ec4141; border-color: #ec4141; }
.panel-close { background: none; border: none; color: var(--text-secondary); font-size: 16px; cursor: pointer; }
.panel-close:hover { color: var(--text-primary); }
.panel-list { flex: 1; overflow-y: auto; padding: 8px 0; }
.panel-empty { text-align: center; color: var(--text-secondary); padding: 80px 0; font-size: 13px; }
...
.pi-name { font-size: 13px; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.pi-name.hl { color: #31c27c; }
.pi-artist { font-size: 11px; color: var(--text-tertiary); }
.pi-fav, .pi-dl, .pi-remove { background: none; border: none; color: var(--text-secondary); font-size: 11px; cursor: pointer; opacity: 0; padding: 2px 4px; border-radius: 3px; }
