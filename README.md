...
| ❤️ 收藏管理 | 乐观更新，即时响应 |
| 📋 歌单管理 | 创建/删除/添加歌曲，完整 CRUD |
| 🕐 最近播放 | 播放历史，自动保留最近 300 条 |
| 🎨 响应式 UI | 白色主题，搜索下拉 + 全屏结果页 |
...
│   ├── src/
│   │   ├── views/          # HomeView, LikesView, PlaylistView, RecentView
│   │   ├── components/     # PlayerBar, PlaylistPopup
│   │   └── api/            # Axios 封装 + 接口定义
│   └── vite.config.js      # Vite 配置 + API 代理
├── vibeMusic-backend/      # Spring Boot 后端
│   └── src/main/java/com/vibemusic/
│       ├── controller/     # Song, Favorite, Playlist, Download
│       ├── service/        # SongService, NeteaseApiService, 缓存/历史
│       ├── entity/         # Song, UserFavorite, Playlist, PlayHistory
│       ├── dto/            # SongDTO
